...
k_matrix = np.zeros((filenum, image_size, image_size))
t_matrix = np.zeros((filenum, image_size, image_size))
...
for fi in range (filenum):
    ...
    kfileIn = "Box0/mymap_box0_s14_is256_new_"+str(fi)+".014.a.z.fits"
    tfileIn = "Box0/mymap_box0_s14_is256_tSZ_"+str(fi)+".014.a.z.fits"

    f0=fits.open(kfileIn)
    k_matrix[fi] = f0[1].data*1000000
    f0.close()

    f0=fits.open(tfileIn)
    t_matrix[fi] = f0[1].data*1000000
    f0.close()
...
x_train_k = k_matrix[0:num_train]
x_train_t = t_matrix[0:num_train]
y_train = velocity_array[0:num_train]

x_test_k = k_matrix[num_train:filenum]
x_test_t = t_matrix[num_train:filenum]
y_test = velocity_array[num_train:filenum]



#print('x_train shape:', x_train_k.shape)
#print(x_train_k.shape[0], 'train samples')
#print(x_test_k.shape[0], 'test samples')


model = Sequential()

ksz = model.add(Conv2D(32, (3, 3), padding='same', input_shape=(image_size, image_size)))(x_train_k)
ksz = model.add(Activation('linear'))(ksz)

ksz = model.add(Conv2D(32, (3, 3), padding = 'same'))(ksz)                            #32 is number of kernell, size of the kernell
ksz = model.add(Activation('linear'))(ksz)
ksz = model.add(MaxPooling2D(pool_size=(2, 2)))(ksz)                  #256*256 to 128*128

ksz = model.add(Conv2D(32, (3, 3), padding='same'))(ksz)
ksz = model.add(Activation('linear'))(ksz)
ksz = model.add(Conv2D(32, (3, 3)))(ksz)
ksz = model.add(Activation('linear'))(ksz)
ksz = model.add(MaxPooling2D(pool_size=(2, 2)))(ksz)
...
tsz = model.add(Conv2D(32, (3, 3), padding='same', input_shape=(image_size, image_size)))(x_train_t)
tsz = model.add(Activation('linear'))(tsz)

tsz = model.add(Conv2D(32, (3, 3), padding = 'same'))(tsz)                            #32 is number of kernell, size of the kernell
tsz = model.add(Activation('linear'))(tsz)
tsz = model.add(MaxPooling2D(pool_size=(2, 2)))(tsz)                  #256*256 to 128*128

tsz = model.add(Conv2D(32, (3, 3), padding='same'))(tsz)
tsz = model.add(Activation('linear'))(tsz)
tsz = model.add(Conv2D(32, (3, 3)))(tsz)
tsz = model.add(Activation('linear'))(tsz)
tsz = model.add(MaxPooling2D(pool_size=(2, 2)))(tsz)
...
x = model.layers.concatenate([ksz, tsz])

x = model.add(Flatten())(x)
x = model.add(Dense(2048))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(512))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(128))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(32))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(8))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(4))(x)
x = model.add(Activation('linear'))(x)
x = model.add(Dense(num_para))(x)
Output = model.add(Activation('linear'))(x)
...
ModelFit = model.fit([x_train_k,x_train_t], y_train,
          batch_size=batch_size,
          epochs=epochs,
          validation_data=([x_test_k,x_test_t], y_test),
          shuffle=True)
