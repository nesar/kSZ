...
model_name = 'ktsz_Zsnap_s'+str(snap)+'-106'
...
f1 = open("cluster_new_0"+str(snap)+".dat")
lines2 = f1.readlines()
...
for fi in range (filenum):
    ...
    kfileIn = "Zsnap_Box0/mymap_box0_s"+str(snap)+"_is256_Zsnap_"+str(fi)+".0"+str(snap)+".a.z.fits"
    tfileIn = "Zsnap_Box0/mymap_box0_s"+str(snap)+"_is256_Zsnap_tSZ_"+str(fi)+".0"+str(snap)+".a.z.fits"

    f0=fits.open(kfileIn)
    kt_matrix[fi, :, :, 0] = f0[1].data*1000000
    f0.close()

    f0=fits.open(tfileIn)
    kt_matrix[fi, :, :, 1] = f0[1].data*1000000
    f0.close()
...
model.add(Conv2D(32, (3, 3), padding = 'same'))                               #32 is number of kernell, size of the kernell
model.add(Activation('linear'))
