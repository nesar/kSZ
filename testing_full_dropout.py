...
for ii in range (len(y_test)/20):
    print ii
    y_pred_do = kdp.predict(x_test[20*ii:(ii+1)*20],n_iter=100)
    y_pred_do_mean = y_pred_do.mean(axis=1)
    y_pred[20*ii:(ii+1)*20]=y_pred_do_mean
    y_pred_do_std = y_pred_do.std(axis=1)
    y_err[20*ii:(ii+1)*20]=y_pred_do_std
